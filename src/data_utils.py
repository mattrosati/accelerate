...
def load_label(patient_id, labels_path, time="seconds"):
    ptid_dict = {fpath.split("_")[0]: fpath for fpath in os.listdir(labels_path) if ".csv" in fpath}
    label_file = os.path.join(labels_path, ptid_dict[patient_id])
...
def build_continuous_time(f, variable_path):
    ...
    step_size = index["frequency"]*1e6 # according to sampling frequency, converts to microseconds
    ...
    start_end = pd.DataFrame({
        "start": (gaps_start_times - index["starttime"].iloc[0].item())/step_size[:-1].reset_index(drop=True),
        "end": (gaps_end_times - index["starttime"].iloc[0].item())/step_size[:-1].reset_index(drop=True)})
    index["zero_start"] = round((index["starttime"] - index["starttime"].iloc[0].item())/step_size)
    mask = np.ones(time.shape[0], dtype=bool)
    # check if time is in any gap
    for i in range(start_end.shape[0]):
        mask[int(start_end["start"].iloc[i].item()):int(start_end["end"].iloc[i].item())] = False
